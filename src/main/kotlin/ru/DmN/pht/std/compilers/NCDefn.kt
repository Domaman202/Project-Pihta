    ...
    override fun compile(node: NodeDefn, compiler: Compiler, ctx: CompilationContext) { // todo: abstract
        ...
        compiler.pushTask(ctx, CompilingStage.METHODS_BODY) {
            method.run {
                val start = Label()
                visitLabel(start)
                val body = BodyContext.of(start, node.method)
                val context = ctx.with(MethodContext(this, node.method)).with(body)
                if (node.method.rettype == VirtualType.VOID) {
                    NCDefault.compile(node, compiler, context)
                    visitInsn(Opcodes.RETURN)
                } else {
                    load(NCDefault.compileVal(node, compiler, context), method)
                    visitInsn(
                        when (node.method.rettype) {
                            VirtualType.BOOLEAN,
                            VirtualType.BYTE,
                            VirtualType.SHORT,
                            VirtualType.CHAR,

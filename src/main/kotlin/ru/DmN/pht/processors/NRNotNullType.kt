...
import ru.DmN.pht.utils.vtype.notNullType
...
object NRNotNullType : IComputableProcessor<NodeNodesList> {
    override fun calc(node: NodeNodesList, processor: Processor, ctx: ProcessingContext): VirtualType? {
        val type = processor.compute(node.nodes[0], ctx)
        return (if (type.isConstClass) processor.computeType(type, ctx) else processor.calc(type, ctx))?.notNullType
    }

...
object NRAs : IStdNodeProcessor<NodeNodesList> {
    ...
    override fun process(node: NodeNodesList, processor: Processor, ctx: ProcessingContext, valMode: Boolean): Node? =
        if (valMode) {
            ...
            if (to !is VTWG && from?.isAssignableFrom(to) == true || from == VOID || from == null)
                processor.process(node.nodes[1], ctx, true)
            else NodeIsAs(node.info.withType(AS_), mutableListOf(processor.process(node.nodes[1], ctx, true)!!), to)
        } else null
}